  ...
  function onVideoPage() {
    ...
    const checkPlayer = setInterval(() => {
      ...
      if (document.body && (document.querySelector('#movie_player') || document.querySelector('.html5-video-player'))) {
        ...
        if (!currentSubtitleStyle || currentSubtitleStyle.showTranslationPrompt !== false) {
          setTimeout(showPrompt, 500);
        }
      } else if (tries > 25) {
        clearInterval(checkPlayer);
        if (document.body && (!currentSubtitleStyle || currentSubtitleStyle.showTranslationPrompt !== false)) {
  ...
  function init() {
    ...
    try {
      chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.action === 'FORCE_START_TRANSLATION') {
          if (!location.pathname.startsWith('/watch')) {
            sendResponse({ success: false, error: 'این دستور فقط در صفحه ویدیو یوتیوب کار می‌کند.' });
            return true;
          }
          translationEnabled = true;
          removePrompt();
