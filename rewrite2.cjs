...
const manifestMatch = content.match(/const manifestJson = \{[\s\S]*?export async function downloadExtensionZip/);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "TRANSLATE_SUBTITLE") {
    ...
    // Relay to sidepanel via port, since sendMessage to sidepanel doesn't wait nicely if not using ports
    // Actually, simple sendMessage from background to sidepanel:
...
<body>
  <iframe id="app-frame" src="\\\${appUrl}" allow="clipboard-write; clipboard-read"></iframe>
...
\\\`;
  ...
  const sidepanelJs = \\\`
// Connect to background to declare we are alive
...
const finalContent = content.substring(0, content.indexOf('const manifestJson')) + newContent + content.substring(content.indexOf('(): Promise<void> {'));
